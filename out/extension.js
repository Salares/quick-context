...
function languageFromExtension(fileName) {
 ...
 */
async function collectFilesFromTargets(targetUris) {
    const files = [];
...
function activate(context) {
    const disposable = vscode.commands.registerCommand("quickContext.generateFromSelection", async (uri, uris) => {
        ...
        for (let i = 0; i < fileUris.length; i++) {
            ...
            const block = `${i + 1}. ${fileName}\n` +
                `${relativePath}\n\n` +
                "```" + fenceLang + "\n" +
                content +
                "\n```\n";
            if (block.length > INPUT_LIMIT_CHARS) {
                vscode.window.showErrorMessage(`Quick Context: File "${relativePath}" alone exceeds the configured input limit (${INPUT_LIMIT_CHARS} characters). Operation aborted.`);
                return;
            }
        ...
        for (let i = 0; i < documents.length; i++) {
            ...
            await editor.edit(edit => edit.insert(new vscode.Position(0, 0), documents[i]));
        ...
        if (documents.length > 1) {
            vscode.window.showInformationMessage(`Quick Context: Generated ${documents.length} prompt documents to stay under the input limit.`);
        }
    });
